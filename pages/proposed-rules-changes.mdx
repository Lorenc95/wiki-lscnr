...
| **Active robbery scene** | Any scripted robbery in progress — store robbery, bank robbery, jewelry heist, or any other scripted heist job. Anyone present during an active robbery is a valid target. |
...
| **Hit** | A player-placed contract targeting a specific player. Placing a hit requires a valid reason (initiation, revenge, etc.). Once active, the target is KOS for all unemployed players, the same as a bounty. Placing a hit without a valid reason is treated as RDM. |
...
| **Kill feeding** | Intentionally allowing another player to kill you to manipulate bounties, stats, or outcomes. |
...
| **Store robbery** | A scripted robbery of a convenience store or similar small business. |
| **Bank robbery** | A scripted robbery of a bank. Counts as an active robbery scene. |
...
**Active hit contract**
If a hit contract has been placed on a player, that player is KOS for all unemployed players — the same rules as a bounty. You must have a valid reason (initiation, revenge, or another reason listed in these rules) to place a hit on someone. Placing a hit without a valid reason is treated as RDM, regardless of who carries it out.
...
During any active scripted robbery — store, bank, jewelry, or any other scripted heist job — all players present are valid targets. Criminals defending the robbery and cops responding to it are both engaging fairly. Bystanders who enter an active scene accept the risk of being engaged.

### b. Revenge

Being attacked, killed, arrested, or having your property damaged or stolen gives you the right to kill the responsible player once within the same game session. This includes failed attempts. If you are attacked again your right to revenge is reinstated.
